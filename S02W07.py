...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  8 12:16:33 2021
...
if DoWeightedNeuralNetwork:
    ...
    if ReTrainWeightedNeuralNetwork:
        ...
        history = modelw.fit(X_train, y_train, epochs=200, batch_size=10, sample_weight= w_train,
                            validation_split=0.2)
        print(history.history.keys())
        plt.clf()
        plt.plot(history.history['accuracy'])
...
PlotLimit(xval,WNNVal, 'Weighted Neural Network', CutVal, r'$m_{ee}^{both}$ fit', 'WNN_Cut')

PlotLimit(xval, SVMVal, 'Support Vector Machine', CutVal, r'$m_{ee}^{both}$ fit', 'SVM_Cut')

PlotLimit(xval, WNNVal, 'Weighted Neural Network', SVMVal, 'Support Vector Machine', 'WNN_SVM')

Plot3Limit(xval, WNNVal, 'Weighted Neural Network', SVMVal, 'Support Vector Machine', CutVal, r'$m_{ee}^{both}$ fit', 'TRIPLE')
